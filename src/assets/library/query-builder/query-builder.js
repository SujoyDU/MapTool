...
Utils.changeType = function(value, type, boolAsInt) {
    switch (type) {
        ...
        case 'datetime':
            //return "ISODate("+new Date(value).toISOString()+")"
...
QueryBuilder.define('sortable', function(options) {
    ...
    this.on('afterInit', function(e) {
        // https://github.com/mistic100/jQuery-QueryBuilder/commit/716612d9111ce906306651379ee85dfd5af04593
...
QueryBuilder.defaults({
    ...
    sqlOperators: {
        ...
        begins_with:      { op: 'begins with ?'      },
        not_begins_with:  { op: 'doesn"t begins with ?' },
        contains:         { op: 'contains ?'  },
        not_contains:     { op: 'doesn"t contains ?'  },
        ends_with:        { op: 'ends with ?' },
        not_ends_with:    { op: 'doesn"t ends with ?' },
        is_empty:         { op: 'is empty' },
        is_not_empty:     { op: 'isn"t empty' },
        is_null:          { op: 'is null' },
        is_not_null:      { op: 'isn"t null '}
    },
    ...
    getRulesFromSQL: function(data, stmt) {
        ...
        (function flatten(data, i) {
            ...
            // it's a leaf
                ...
                // convert array
                var value;
                if ($.isArray(data.left.value)) {
                    value = data.left.value.map(function(v) {
                        return v.value;
                    });
                }
                else {
                ...
                curr.rules.push({
                    id: that.change('getSQLFieldID', data.left.value, data.left.value),
